...
控制器通过光耦输入板支持 8 个 NPN 型 SN04-N 限位信号。光耦板的 ESP32
侧输出必须是 3.3 V 安全电平或开集电极输出，绝不能把传感器的 24 V
输出直接接到 ESP32 GPIO。
...
按常见 NPN 接法，棕线接 +24 V，蓝线接 0 V，黑线接对应传感器输入。
当前这块光耦板在 ESP32 侧使用 `Pin.PULL_DOWN`，高电平表示触发；这是
根据空闲状态实测确认的输出极性。更换光耦板后必须重新确认极性。Y 轴任一
侧触发后，只禁止继续朝该方向运动，反向脱离仍然允许。

程序在每段运动开始前以及运动中每 4 个脉冲检查限位。软件限位不能替代
硬件急停，设备仍必须保留独立的硬件急停回路。
